        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {items.map((item, idx) => {
                  const effectiveAvg = item.isCustomRow
                    ? (parseInt((item.avgPriceCustom || '').replace(/,/g, '')) || 0)
                    : item.avgPrice;
                  const effectivePrice = item.customPrice !== ''
                    ? parseInt(item.customPrice.replace(/,/g, '')) || 0
                    : effectiveAvg;
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-center" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          onFocus={() => { if (!item.checked) onToggle(item.id); }}
                      ...
                      <td className="px-3 py-2 text-center">
                        {item.isCustomRow ? (
                          <button
                            onClick={(e) => { e.stopPropagation(); onRemoveRow(item.id); }}
                ...
                <tr className={`border-t border-gray-100`}>
                  <td colSpan={5} className="px-4 py-2">
                    <button
                      ...
                      className={`text-xs font-semibold px-3 py-1.5 rounded-lg border-2 border-dashed transition-colors
